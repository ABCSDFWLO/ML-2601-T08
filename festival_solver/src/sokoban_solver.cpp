...
void attrPrep(pthread_attr_t *iattr)
{
//note that OSX has a tiny default pthread-stacksize 
//that MUST be enlarged in order to avoid a segfault !
//Note: 8Mb=linux default (osx_default=0.5Mb)
//Note: this prep does no harm in linux or Windows.

	pthread_attr_init(iattr);
	pthread_attr_setstacksize(iattr, 8388608); 
	pthread_attr_setdetachstate(iattr, PTHREAD_CREATE_JOINABLE);
...
void solve_with_time_control_two_cores(board b)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		rc0 = pthread_create(&threads[0], &attr, solve_work_element, &we_0);
		rc1 = pthread_create(&threads[1], &attr, solve_work_element, &we_1);

		rc0=pthread_join(threads[0], NULL);
		rc1=pthread_join(threads[1], NULL);
...
void solve_with_time_control_four_cores(board b)
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		for (j = 0 ; j < 4 ; j++)
			rc[j] = pthread_create(&threads[j], &attr, solve_work_element, we + j);
...
//free(dragonfly_nodes); //fastrgv (didn't see elsewhere)
