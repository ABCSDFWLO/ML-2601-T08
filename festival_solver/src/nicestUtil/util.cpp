...
int get_number_of_cores()
{
	...
	//memory = getTotalSystemMemoryMb();
	memory = getAvailSystemMemoryMb();
...
//fastrgv
	...
	//processor_count=1; //continue so memory gets printed!
	size_t size = sizeof(processor_count);
	sysctlbyname("hw.physicalcpu", &processor_count, &size, NULL, 0);
	// this needs to include: <sys/types.h> <sys/sysctl.h> on OSX

	// festival segfaults later on when using > 1 thread ?
	processor_count=1;
