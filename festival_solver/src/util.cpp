...
//fastrgv
	...
	#include <termios.h> //osx
	#include <unistd.h>
	#include <sys/sysctl.h>
//#else //MsWin
#elif defined _WIN64
	#include <conio.h> //MsWin
	#include <windows.h>
#else // ...add more here?
...
//begin fastrgv addenda..............................................



#include "memory.hpp" // all 3 OS
int get_number_of_cores()
{
	int processor_count;
	unsigned long long memory, memoryMb;

	cores_num = 1;

	//memoryMb = getTotalSystemMemoryMb();
	memoryMb = getAvailSystemMemoryMb();

	memory = (int)((memoryMb/1024) + 0.5);
...
//fastrgv
	...
	sysctlbyname("hw.physicalcpu", &processor_count, &size, NULL, 0);
...
void join2path(const char *tail, char *str)
{
#ifdef __linux__
	sprintf(str, "%s/%s", global_dir, tail);
#elif defined __APPLE__ && defined __MACH__
	sprintf(str, "%s/%s", global_dir, tail);
#elif defined _WIN64
	sprintf(str, "%s\\%s", global_dir,tail);
